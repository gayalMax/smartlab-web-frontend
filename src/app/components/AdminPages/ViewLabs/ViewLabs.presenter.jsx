...
function ViewLabsPresenter({ classes, labs, error, loading, onRefresh }) {
  ...
  const openDialog = lab => () => setLabManagers(lab.managers);
        ...
        <Grid container direction="column" alignItems="stretch" className={classes.wrapper}>
          ...
          <Grid item>
            <AdvancedTable
                ...
                },
                {
                  ...
                  render: row => (
                    <Button
                      color="secondary"
                      variant="contained"
                      disabled={row.managers.length === 0}
              ...
              data={labs.map(({ id, title, subtitle, image, createdAt, updatedAt, managers }) => ({
                id,
                title,
                subtitle,
                image,
